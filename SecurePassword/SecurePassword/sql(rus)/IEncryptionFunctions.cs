...
internal interface IEncryptionFunctions
{
    ...
    abstract public static byte[] PackDEK(byte[] nonce, byte[] tag, byte[] ciphertext); //Объединение данных для шифровки DEK
    abstract public static void UnpackDEK(byte[] pack, out byte[] nonce, out byte[] tag, out byte[] ciphertext); //Чтение шифрованного DEK 
}